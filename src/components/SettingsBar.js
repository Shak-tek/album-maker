...
export default function SettingsBar({
  ...
  onSave,
  onEditTitle,
  fileInputRef,                 // <-- you already had this
  textSettings,                 // <-- NEW: { fontFamily, fontSize, color }
  onChangeTextSettings,         // <-- NEW: setter
}) {
  ...
  const handleFiles = (e) => {
    ...
    if (files.length && typeof onAddImages === 'function') onAddImages(files);
    e.target.value = '';
  };
  ...
  const ts = {
    fontFamily: textSettings?.fontFamily || fonts[0],
  ...
  return (
    <Box className="settings-bar" direction="row" gap="small" pad="xsmall" background="light-1" elevation="medium" align="center">
      ...
      {/* Theme */}
      {onOpenThemeModal && (
        <Button className="btn-setting" label="Change Theme" onClick={onOpenThemeModal} />
      )}
      ...
      {/* Save */}
      {onSave && (
        <Button className="btn-setting" label="Save" onClick={onSave} />
      )}

      {/* --- NEW: Text controls --- */}
      <Box direction="row" gap="xsmall" align="center" pad={{ horizontal: 'small' }} className="text-controls" border={{ side: 'left', color: 'light-4' }}>
        <Text size="small" weight="bold" margin={{ right: 'xsmall' }}>Text</Text>

        {/* Font family */}
        <select
          aria-label="Font family"
          value={ts.fontFamily}
          onChange={(e) => setTS({ fontFamily: e.target.value })}
          style={{ maxWidth: 220 }}
        >
          {fonts.map(f => <option key={f} value={f}>{f.split(',')[0].replaceAll('"', '')}</option>)}
        </select>
        ...
        <input
          ...
          value={ts.fontSize}
          onChange={(e) => setTS({ fontSize: Math.max(8, Math.min(72, Number(e.target.value) || 8)) })}
          style={{ width: 70 }}
        />
